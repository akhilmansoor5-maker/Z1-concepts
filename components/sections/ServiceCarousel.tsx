...
export function ServiceCarousel({ kicker, heading, copy, items }: ServiceCarouselProps) {
  ...
  return (
    <section className="border-t border-white/10 bg-z1-black py-12 sm:py-16">
      <Container>
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <Reveal>
            ...
            <h2 className="mt-3 max-w-xl font-display text-[clamp(1.65rem,6vw,2.75rem)] leading-[0.95] font-semibold uppercase tracking-tight">
              {heading}
            </h2>
        ...
        <div
          ref={scrollerRef}
          className="mt-7 flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain pb-2 [-webkit-overflow-scrolling:touch] [scrollbar-width:none] touch-pan-x [&::-webkit-scrollbar]:hidden"
        >
          {items.map((item, itemIndex) => (
            <article
              key={item.name}
              data-card
              className="w-[min(17.5rem,78vw)] shrink-0 snap-start border border-white/10 bg-z1-charcoal sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
            >
              ...
              <div className="p-4 sm:p-5">
                ...
                <h3 className="mt-2 font-display text-lg uppercase tracking-tight sm:text-xl">
                  {item.name}
                </h3>
                <p className="mt-2 text-[15px] leading-relaxed text-white/55">{item.copy}</p>
                <div className="mt-5">
                  <Button
